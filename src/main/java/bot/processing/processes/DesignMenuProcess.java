...
public class DesignMenuProcess extends Process{
    private CheckBoxKeyBoardProvider checkBoxBusinessMenu;

    public DesignMenuProcess(UserSession userSession) {
        this.userSession = userSession;
        List<String> types = ListsUtil.getList("designMenu.txt");
        // boolean[] selected = loadSelectedTypes(userSession.getChatId(), types);

        checkBoxBusinessMenu = new CheckBoxKeyBoardProvider(types, "designMenu");
        ...
        switch (curState) {
            case "sendQuestion": {
                ChatBot.sendMessage(userSession.getChatId(),
                        "Вы выбрали дизайн, хотите уточнить отрасли?");
