...
public class MarketingMenuProcess extends Process{
    private CheckBoxKeyBoardProvider checkBoxBusinessMenu;

    public MarketingMenuProcess(UserSession userSession) {
        this.userSession = userSession;
        List<String> types = ListsUtil.getList("marketingMenu.txt");
        // boolean[] selected = loadSelectedTypes(userSession.getChatId(), types);

        checkBoxBusinessMenu = new CheckBoxKeyBoardProvider(types, "marketingMenu");
        ...
        switch (curState) {
            case "sendQuestion": {
                ChatBot.sendMessage(userSession.getChatId(),
                        "Вы выбрали маркетинг, хотите уточнить отрасли?");
