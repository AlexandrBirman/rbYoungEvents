...
public class DevelopmentMenuProcess extends Process{
    private CheckBoxKeyBoardProvider checkBoxBusinessMenu;

    public DevelopmentMenuProcess(UserSession userSession) {
        this.userSession = userSession;
        List<String> types = ListsUtil.getList("developmentMenu.txt");
        // boolean[] selected = loadSelectedTypes(userSession.getChatId(), types);

        checkBoxBusinessMenu = new CheckBoxKeyBoardProvider(types, "developmentMenu");
        ...
        switch (curState) {
            case "sendQuestion": {
                ChatBot.sendMessage(userSession.getChatId(),
                        "Вы выбрали разработка, хотите уточнить отрасли?");
